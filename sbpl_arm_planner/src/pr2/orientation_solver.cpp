...
bool RPYSolver::isOrientationFeasible(const double* rpy, std::vector<double> &start, std::vector<double> &prefinal, std::vector<double> &final)
{
  ...
  {
    SBPL_DEBUG("computeFK failed on forearm pose.\n");
    return false;
  }
  ...
  {
    SBPL_DEBUG("computeFK failed on end_eff pose.\n");
    return false;
  }

#if DEBUG
  SBPL_DEBUG_NAMED("orientation_solver", "Joint Angles: %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f", start[0], start[1], start[2], start[3], start[4], start[5], start[6]);
  SBPL_DEBUG_NAMED("orientation_solver", "FK:forearm: %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f",forearm_pose[0], forearm_pose[1], forearm_pose[2], forearm_pose[3], forearm_pose[4], forearm_pose[5], forearm_pose[6]);
  SBPL_DEBUG_NAMED("orientation_solver", "FK:endeff:  %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f",endeff_pose[0], endeff_pose[1], endeff_pose[2], endeff_pose[3], endeff_pose[4], endeff_pose[5], endeff_pose[6]);
  SBPL_DEBUG_NAMED("orientation_solver", "Start yaw: %0.4f, Start pitch: %0.4f, Start roll: %0.4f",endeff_pose[5], endeff_pose[4], endeff_pose[3]);
  SBPL_DEBUG_NAMED("orientation_solver", "End yaw required: %0.4f, End pitch required: %0.4f, End roll required: %0.4f", rpy[2], rpy[1], rpy[0]);
#endif
...
#if DEBUG
  SBPL_DEBUG_NAMED("orientation_solver", "[isGoalPositionGokul] The rotations commanded in the first attempt are: %0.4f, %0.4f and %0.4f", hand_rotations[1], hand_rotations[2], hand_rotations[3]);
#endif

  if(hand_rotations[0]==0)
  {
#if DEBUG
    SBPL_DEBUG_NAMED("orientation_solver", "Orientation solver predicts that this movement is impossible with the given pitch limits.");
#endif
...
#if DEBUG
  SBPL_DEBUG_NAMED("orientation_solver", "\n \n After:");
  arm_->computeFK(final,END_EFF,endeff_pose);
  SBPL_DEBUG_NAMED("orientation_solver", "The perfect roll value from FK is: %0.4f.", rpy[0]-endeff_pose[3]);

  SBPL_DEBUG_NAMED("orientation_solver", "Joint angles:%0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f", final[0], final[1], final[2], final[3], final[4], final[5], final[6]);

  SBPL_DEBUG_NAMED("orientation_solver", "Forward kinematics on final arm position.");
  arm_->computeFK(final,FOREARM_ROLL,forearm_pose);
  SBPL_DEBUG_NAMED("orientation_solver", "FK:f_arm: %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f",forearm_pose[0], forearm_pose[1], forearm_pose[2], forearm_pose[3], forearm_pose[4], forearm_pose[5], forearm_pose[6]);
  arm_->computeFK(final,END_EFF,endeff_pose);
  SBPL_DEBUG_NAMED("orientation_solver", "FK:endeff: %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f, %0.4f",endeff_pose[0], endeff_pose[1], endeff_pose[2], endeff_pose[3], endeff_pose[4], endeff_pose[5], endeff_pose[6]);

  SBPL_DEBUG_NAMED("orientation_solver", "Succeeded.");
#endif
