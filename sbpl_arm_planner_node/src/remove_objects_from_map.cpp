#include <sbpl_arm_planner_node/remove_objects_from_map.h>


RemoveObjectsFromMap::RemoveObjectsFromMap()
{
  object_in_map_pub_ = nh_.advertise<arm_navigation_msgs::CollisionObject>("collision_object", 1024);
}

bool RemoveObjectsFromMap::parseObjectsFile(FILE* fCfg, std::vector<std::vector<double> > &objects, std::vector<std::string> &object_ids)
{
  char sTemp[1024];
  int num_obs = 0;
...
void RemoveObjectsFromMap::removeObjectsFromFile(std::string filename)
{
  ...
  removeBoxes(objects_, object_ids_);
}

void RemoveObjectsFromMap::removeBoxes(std::vector<std::vector<double> > &objects)
{
  ...
  removeBoxes(objects, object_ids);
}

void RemoveObjectsFromMap::removeBoxes(std::vector<std::vector<double> > &objects, std::vector<std::string> &object_ids)
{
  ...
  for(size_t i = 0; i < objects.size(); i++)
  {
      ...
      removeBox(pose, dims, object_ids.at(i));
  }
}

void RemoveObjectsFromMap::removeBox(geometry_msgs::Pose pose, std::vector<double> &dims, std::string id)
{
...
void RemoveObjectsFromMap::printObjects(FILE * fOut)
{
...
{
  ros::init(argc,argv,"remove_object_from_map");
  sleep(2);
  RemoveObjectsFromMap removeobj;

  ROS_INFO("removing known objects from map 1 times.");
  int i = 0;
  while(i<1)
  { 
    if(argc > 1)
      removeobj.removeObjectsFromFile(std::string(argv[1]));
    else
      removeobj.removeObjectsFromFile(std::string("objects.txt"));

    ROS_INFO("%d: removed objects from collision map", i);
