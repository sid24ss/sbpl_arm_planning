...
SBPLCollisionModel::SBPLCollisionModel() : ph_("~") 
{
  urdf_.reset();
  dgroup_ = NULL;
  chain_root_name_ = "base_footprint";
}
...
bool SBPLCollisionModel::initKDLChainForGroup(std::string &chain_root_name, Group* group)
{
  ...
  ROS_INFO("A chain was initialized for the %s from %s to %s.", group->name_.c_str(), chain_root_name_.c_str(), group->chain_.getSegment(group->chain_.getNrOfSegments()-1).getName().c_str());
  return true;
}
...
bool SBPLCollisionModel::initKDLChainForGroup(Group* group)
{
  ...
  while(unincluded_links)
  {
    ...
    for(size_t i = 0; i < group->links_.size(); ++i)
    {
      ...
      if (!kdl_tree_.getChain(chain_root_name_, group->links_[i].root_name_, chain))
      {
        ROS_ERROR("Error: could not fetch the KDL chain for the desired manipulator. Exiting. (root: %s, tip: %s)", chain_root_name_.c_str(), group->links_[i].root_name_.c_str());
        continue;
      }
    ...
    if (!kdl_tree_.getChain(chain_root_name_, group->links_[i_max].root_name_, chain))
    {
      ROS_ERROR("Error: could not fetch the KDL chain for the desired manipulator. Exiting. (root: %s, tip: %s)", chain_root_name_.c_str(), group->links_[i_max].root_name_.c_str());
      continue;
    }
...
bool SBPLCollisionModel::initGroup(Group* group)
{
  ...
  // debug output
  ROS_INFO("[%s] Frames:", group->name_.c_str());
  for(size_t i = 0; i < group->frames_.size(); ++i)
    for(size_t j = 0; j < group->frames_[i].size(); ++j)
      ROS_INFO("[%s] [chain %d] segment: %d", group->name_.c_str(), int(i), group->frames_[i][j]);
  ...
  // debug output
  ROS_INFO("[%s] Order of Joints in Joint Array input to the FK Solver:", group->name_.c_str());
  for(size_t i = 0; i < group->jntarray_names_.size(); ++i)
    for(size_t j = 0; j < group->jntarray_names_[i].size(); ++j)
      ROS_INFO("[%s] [chain %d] %d: %s", group->name_.c_str(), int(i), int(j), group->jntarray_names_[i][j].c_str());
...
void SBPLCollisionModel::setOrderOfJointPositions(const std::vector<std::string> &joint_names, std::string group_name)
{
  ...
  for(size_t i = 0; i < joint_names.size(); ++i)
  {
    ...
    ROS_INFO("[%s] [%d] %s", group_name.c_str(), int(i), joint_names[i].c_str());
  ...
  for(size_t i = 0; i < group->angles_to_jntarray_.size(); ++i)
    for(size_t j = 0; j < group->angles_to_jntarray_[i].size(); ++j)
      ROS_INFO("[%s] [chain %d] joint: %s  index: %d", group_name.c_str(), int(i), joint_names[j].c_str(), group->angles_to_jntarray_[i][j]);
}
...
int main(int argc, char **argv)
{
  ...
  names[0] = "r_wrist_roll_joint";
  names[1] = "r_upper_arm_roll_joint";
  names[2] = "torso_lift_joint";
  names[6] = "r_elbow_flex_joint";
  names[3] =  "r_forearm_roll_joint";
  names[4] = "r_wrist_flex_joint";
  names[5] = "r_gripper_l_finger_joint";

  ROS_INFO(" SETTING THE JOINT POSITIONS NOW ");
  model.setOrderOfJointPositions(names, "right_arm");
  
  model.setDefaultGroup("right_arm");

  model.printDebugInfo("right_arm");

  ROS_INFO("JUST SET THE DEFAULT GROUP, NOW RUNNING FK");
